...
------------------------------------------------------------------------
local my_MaskZeroCriterion, parent = torch.class("nn.MyMaskZeroCriterion", "nn.MyCriterion")

function my_MaskZeroCriterion:__init(criterion, nInputDim)
   parent.__init(self)
   self.criterion = criterion
   assert(torch.isTypeOf(criterion, 'nn.MyCriterion'))
   assert(torch.type(nInputDim) == 'number', 'Expecting nInputDim number at arg 2')
   self.nInputDim = nInputDim
end

function my_MaskZeroCriterion:recursiveGetFirst(input)
...
function my_MaskZeroCriterion:recursiveMask(dst, src, mask)
...
function my_MaskZeroCriterion:updateOutput(input, target, rewards)   
   ...
   if j > 0 then
      ...
      self.output = self.criterion:updateOutput(self.input, self.target, rewards)
   else
...
function my_MaskZeroCriterion:recursiveMaskGradInput(dst, mask, src, input)
   if torch.type(input) == 'table' then
   ...
   elseif torch.isTensor(input) then
      ...
      dst:resizeAs(input:double()):zero()
...
function my_MaskZeroCriterion:updateGradInput(input, target, rewards)
   if self.zeroMask:nElement() > 0 then
      assert(self.input and self.target)
      self._gradInput = self.criterion:updateGradInput(self.input, self.target, rewards)
   end
   self.gradInput = self:recursiveMaskGradInput(self.gradInput, self.zeroMask, self._gradInput, input)
   return self.gradInput
end

function my_MaskZeroCriterion:type(type, ...)
   self.zeroMask = nil
   self._zeroMask = nil
   self.__zeroMask = nil
   self.input = nil
