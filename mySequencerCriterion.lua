...
------------------------------------------------------------------------
local my_sequencerCriterion, parent = torch.class('nn.MySequencerCriterion', 'nn.MyCriterion')

function my_sequencerCriterion:__init(criterion)
   parent.__init(self)
...
function my_sequencerCriterion:getStepCriterion(step)
...
function my_sequencerCriterion:updateOutput(input, target, rewards)
   self.output = 0
   local nStep
   ...
   for i=1,nStep do
      ...
      self.output = self.output + criterion:forward(input[i], target[i], rewards[i])
   end
   
   return self.output
end

function my_sequencerCriterion:updateGradInput(input, target, rewards)
   self.gradInput = {}
   if torch.isTensor(input) then
      assert(torch.isTensor(target), "expecting target Tensor since input is a Tensor")
      assert(target:size(1) == input:size(1), "target should have as many elements as input")
   ...
   for i=1,nStep do
      local criterion = self:getStepCriterion(i)
      tableGradInput[i] = criterion:backward(input[i], target[i], rewards[i])
   end
