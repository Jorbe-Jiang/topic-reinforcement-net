...
cmd:option('-topic_num_size', 100, 'the number of the topics') --if load model, then must be same with loaded model
cmd:option('-top_k_actions', 1000, 'every time Policy net can choose top k actions to calculate the loss')
...
--others
cmd:option('-version', 'HRED model+topic_reinforcement_net: use wikipedia word vector to init and train the word vector with model', 'depict of this version demo')
...
function train_model()
	...
	require 'build_model'
	local model, policy_net_criterion = build()
	...
	train(model, policy_net_criterion, batches_train_data, batches_valid_data)
end
